...
describe("addProperty (Server Action)", () => {
  ...
  it("creates a property with no loan block", async () => {
    ...
    const { data: props } = await user.client
      .from("properties")
      .select("*")
      .eq("address", "42 No Loan Ave");
    ...
    const { data: loans } = await user.client
      .from("loans")
      .select("*")
      .eq("property_id", props![0].id);
