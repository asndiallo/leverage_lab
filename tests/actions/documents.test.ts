...
describe("document Server Actions", () => {
  ...
  beforeAll(async () => {
    ...
    const txn = await createTestTransaction(user.client, user.id, propertyId, { category: "rent" });
  ...
  it("rejects a non-PDF/image mime type", async () => {
    ...
    const result = await uploadDocument(emptyActionState, buildFormData({
      property_id: propertyId, doc_type: "other", file,
    }));
  ...
  it("rejects a file over 15MB", async () => {
    ...
    const result = await uploadDocument(emptyActionState, buildFormData({
      property_id: propertyId, doc_type: "other", file: big,
    }));
  ...
  it("uploads a document, records it, and optionally links it to a transaction", async () => {
    ...
    const result = await uploadDocument(emptyActionState, buildFormData({
      property_id: propertyId, transaction_id: transactionId, doc_type: "closing_disclosure", file,
    }));
    ...
    const { data: signed } = await user.client.storage.from("documents").createSignedUrl(doc.storage_path, 60);
    expect(signed?.signedUrl).toBeTruthy();
  });

  it("links and unlinks an already-uploaded document to a transaction separately", async () => {
    await actAs(user);
    const upload = await uploadDocument(emptyActionState, buildFormData({
      property_id: propertyId, doc_type: "other", file: pdfFile("standalone.pdf"),
    }));
    ...
    const linked = await linkDocument(emptyActionState, buildFormData({
      document_id: doc!.id, transaction_id: transactionId, property_id: propertyId,
    }));
    ...
    const unlinked = await unlinkDocument(emptyActionState, buildFormData({
      link_id: link!.id, property_id: propertyId, transaction_id: transactionId,
    }));
  ...
  it("deletes a document and its storage object", async () => {
    ...
    await uploadDocument(emptyActionState, buildFormData({
      property_id: propertyId, doc_type: "other", file: pdfFile("to-delete.pdf"),
    }));
    const { data: doc } = await user.client
      .from("documents")
    ...
    const result = await deleteDocument(emptyActionState, buildFormData({ document_id: doc!.id }));
    expect(result.error).toBeUndefined();

    const { data: afterDelete } = await user.client.from("documents").select("id").eq("id", doc!.id);
