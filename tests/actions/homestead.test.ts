...
import { createTestProperty, createTestJurisdiction, createTestExemption, deleteTestProperty } from "../setup/fixtures";
...
describe("setHomesteadFiled (Server Action)", () => {
  ...
  beforeAll(async () => {
    ...
    const j1 = await createTestJurisdiction(user.client, user.id, propertyId, { name: "Test ISD" });
    const j2 = await createTestJurisdiction(user.client, user.id, propertyId, { name: "Test County" });
    await createTestExemption(user.client, user.id, propertyId, j1.id, {
      exemption_type: "homestead", applied: false,
    });
    ...
    await createTestExemption(user.client, user.id, propertyId, j2.id, {
      exemption_type: "over65", applied: false,
    });
  });
  ...
  it("marks every homestead exemption on the property as applied", async () => {
    await actAs(user);
    const result = await setHomesteadFiled(emptyActionState, buildFormData({
      property_id: propertyId, filed: "true",
    }));
  ...
  it("can toggle back to not-filed", async () => {
    await actAs(user);
    const result = await setHomesteadFiled(emptyActionState, buildFormData({
      property_id: propertyId, filed: "false",
    }));
    expect(result.error).toBeUndefined();
